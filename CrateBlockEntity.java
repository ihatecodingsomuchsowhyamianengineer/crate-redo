...
import com.novaventure.survivalessentials.registry.Blocks.Decorations.CrateBlock;
...
import static com.novaventure.survivalessentials.SurvivalEssentials.CRATE_BLOCK;
...
public class CrateBlockEntity extends LootableContainerBlockEntity {
    ...
    public CrateBlockEntity(BlockEntityType<?> type) {
        super(type);
    ...
    protected Text getContainerName() {
        return new TranslatableText("container.crate");
    }
    ...
    public void onOpen(PlayerEntity player) {
        if (!player.isSpectator()) {
            ...
            boolean bl = (Boolean)blockState.get(CrateBlock.OPEN);
            if (!bl) {
    ...
    public void tick() {
        ...
        if (this.viewerCount > 0) {
            ...
            BlockState blockState = this.getCachedState();
            if (!blockState.isOf(CRATE_BLOCK)) {
                this.markRemoved();
                return;
            }
    ...
    private void playSound(BlockState blockState, SoundEvent soundEvent) {
        Vec3i vec3i = ((Direction)blockState.get(CrateBlock.FACING)).getVector();
